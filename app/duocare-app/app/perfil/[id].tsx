    ...
    <View style={styles.postCard}>
      <View style={styles.postHeader}>
        <TouchableOpacity onPress={navegarParaPerfil}>
          {post.fotoUsuario ? (
            <Image source={{ uri: post.fotoUsuario }} style={styles.avatar} />
          ) : (
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>{(post.nomeUsuario || '?').charAt(0).toUpperCase()}</Text>
            </View>
          )}
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
...
export default function PerfilUsuarioScreen() {
  ...
  useEffect(() => { carregarPerfil(); }, [userId]);
  useEffect(() => { if (!isOwnProfile && userId && user?.userId) carregarStatusConexao(); }, [userId, user?.userId]);
  ...
  async function carregarStatusConexao() {
    try {
      ...
      if (isConnected) { setConexaoStatus('connected'); return; }
      const temSolicitacaoRecebida = pendentesRecebidas.some(c => c.userId === userId);
      if (temSolicitacaoRecebida) { setConexaoStatus('pending_received'); return; }
      const temSolicitacaoEnviada = pendentesEnviadas.some(c => c.userId === userId);
      if (temSolicitacaoEnviada) { setConexaoStatus('pending_sent'); return; }
  ...
  const handleCurtir = async (postId: number) => {
    try {
      ...
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, curtidoPorMim: updatedPost.curtidoPorMim, totalCurtidas: updatedPost.totalCurtidas } : p));
      setLikedPosts(prev => prev.map(p => p.id === postId ? { ...p, curtidoPorMim: updatedPost.curtidoPorMim, totalCurtidas: updatedPost.totalCurtidas } : p));
    } catch (error) {
  ...
  const handleComentar = (postId: number) => {
    ...
    setLikedPosts(prev => prev.map(p => p.id === postId ? { ...p, totalComentarios: p.totalComentarios + 1 } : p));
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <View style={styles.userSection}>
          <AvatarGrande nome={profile.nome} fotoUrl={profile.fotoUrl} size={80} />
          ...
          {!isOwnProfile && (
            <View style={styles.conexaoButtonContainer}>
              {conexaoStatus === 'connected' && (
                <TouchableOpacity style={[styles.btnConectar, styles.btnDesconectar]} onPress={handleDesconectar} disabled={acaoConexaoLoading}>
                  <Text style={styles.btnConectarText}>Conectado</Text>
                </TouchableOpacity>
              )}
              {conexaoStatus === 'pending_received' && (
                <TouchableOpacity style={[styles.btnConectar, styles.btnAceitar]} onPress={handleAceitarConexao} disabled={acaoConexaoLoading}>
                  <Text style={styles.btnConectarText}>Aceitar solicitação</Text>
                </TouchableOpacity>
              )}
              {conexaoStatus === 'pending_sent' && (
                <TouchableOpacity style={[styles.btnEnviado]} onPress={handleCancelarSolicitacao} disabled={acaoConexaoLoading}>
                  <Text style={styles.btnEnviadoText}>Solicitação enviada</Text>
                </TouchableOpacity>
              )}
              {conexaoStatus === 'none' && (
                <TouchableOpacity style={styles.btnConectar} onPress={handleSolicitarConexao} disabled={acaoConexaoLoading}>
        ...
        <View style={styles.statsGrid}>
          <StatCard icon={<IconStar size={22} color={colors.accent} />} value={profile.pontos.toLocaleString()} label="Pontos" color={colors.accent} delay={0} />
          <StatCard icon={<IconTrophy size={22} color={profile.ligaCor || colors.accent} />} value={profile.ligaNome} label="Liga" color={profile.ligaCor || colors.accent} delay={100} />
          <StatCard icon={<IconUsers size={22} color={colors.secondary} />} value={conexoesCount.toLocaleString()} label="Conexões" color={colors.secondary} delay={200} />
        </View>

        <View style={styles.tabsContainer}>
          <TouchableOpacity style={[styles.tabButton, activeTab === 'posts' && styles.tabButtonActive]} onPress={() => setActiveTab('posts')}>
            <Ionicons name="grid-outline" size={28} color={activeTab === 'posts' ? colors.primary : colors.textSecondary} />
          </TouchableOpacity>
          <TouchableOpacity style={[styles.tabButton, activeTab === 'likes' && styles.tabButtonActive]} onPress={() => setActiveTab('likes')}>
            <Ionicons name="heart-outline" size={28} color={activeTab === 'likes' ? colors.primary : colors.textSecondary} />
          </TouchableOpacity>
        </View>

        {currentList.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Ionicons name={activeTab === 'posts' ? 'images-outline' : 'heart-outline'} size={48} color={colors.textLight} />
            <Text style={styles.emptyText}>{activeTab === 'posts' ? 'Nenhuma publicação ainda.' : 'Nenhuma curtida ainda.'}</Text>
          </View>
        ) : (
          currentList.map((post) => (
            <PostItem key={post.id} post={post} onCurtir={handleCurtir} onComentar={handleComentar} />
          ))
        )}
      </ScrollView>
    </SafeAreaView>
...
// ─── Estilos (mantidos iguais, mas com ajuste no avatar para suportar imagem) ───
const styles = StyleSheet.create({
  ...
  userSection: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: colors.background, paddingHorizontal: 20, paddingVertical: 20, borderBottomWidth: 1, borderBottomColor: colors.border },
  avatarGrande: { backgroundColor: colors.primaryMuted, alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: colors.primary + '44', overflow: 'hidden' },
  ...
  ligaChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, borderWidth: 1, alignSelf: 'flex-start' },
  ligaChipText: { fontSize: 13, fontWeight: '600' },
  btnEditar: { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1.5, borderColor: colors.primary, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6 },
  btnEditarText: { color: colors.primary, fontWeight: '600', fontSize: 13 },
  conexaoButtonContainer: { minWidth: 140, alignItems: 'flex-end' },
  btnConectar: { backgroundColor: colors.primary, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 30, alignItems: 'center', justifyContent: 'center' },
  btnDesconectar: { backgroundColor: colors.errorMuted, borderWidth: 1, borderColor: colors.error },
  btnAceitar: { backgroundColor: colors.success },
  btnEnviado: { backgroundColor: colors.border, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 30, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.textSecondary },
  btnConectarText: { color: colors.white, fontWeight: '600', fontSize: 13 },
  btnEnviadoText: { color: colors.textSecondary, fontWeight: '600', fontSize: 13 },
  ...
  statCard: { width: '30%', backgroundColor: colors.surface, borderRadius: 16, padding: 14, alignItems: 'center', gap: 8, borderWidth: 0, borderBottomWidth: 2 },
  ...
  tabsContainer: { flexDirection: 'row', justifyContent: 'center', gap: 32, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border },
  tabButton: { paddingHorizontal: 20, paddingVertical: 8, borderRadius: 20 },
  tabButtonActive: { borderBottomWidth: 2, borderBottomColor: colors.primary },
  postCard: { backgroundColor: colors.surface, borderRadius: 20, padding: 16, marginHorizontal: 16, marginTop: 12, gap: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 6, elevation: 3 },
  postHeader: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  avatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: colors.primaryMuted, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  avatarText: { fontSize: 18, fontWeight: '700', color: colors.primary },
  postNome: { fontSize: 16, fontWeight: '700', color: colors.text },
  postData: { fontSize: 12, color: colors.textLight, marginTop: 2 },
  postConteudo: { fontSize: 15, lineHeight: 22, color: colors.text },
  postActions: { flexDirection: 'row', gap: 24, marginTop: 8, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 12 },
  ...
  commentInputContainer: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: colors.surfaceHigh, borderRadius: 24, paddingHorizontal: 12, paddingVertical: 6, marginBottom: 12 },
  commentInput: { flex: 1, color: colors.text, fontSize: 14, maxHeight: 80 },
  commentItem: { flexDirection: 'row', gap: 10, marginBottom: 12 },
  commentAvatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: colors.primaryMuted, alignItems: 'center', justifyContent: 'center' },
  commentAvatarText: { fontSize: 14, fontWeight: '700', color: colors.primary },
  commentNome: { fontSize: 14, fontWeight: '600', color: colors.text },
  commentConteudo: { fontSize: 13, color: colors.textSecondary, marginTop: 2 },
  commentData: { fontSize: 10, color: colors.textLight, marginTop: 2 },
  emptyComments: { textAlign: 'center', color: colors.textLight, fontSize: 12, marginTop: 12 },
