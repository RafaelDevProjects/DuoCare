    ...
    <View style={styles.postCard}>
      ...
      <View style={styles.postActions}>
        <TouchableOpacity style={styles.actionButton} onPress={() => onCurtir(post.id)}>
          <Ionicons name={post.curtidoPorMim ? 'heart' : 'heart-outline'} size={22} color={post.curtidoPorMim ? colors.error : colors.textSecondary} />
              ...
              renderItem={({ item }) => (
                <View style={styles.commentItem}>
                  <View style={styles.commentAvatar}>
                    <Text style={styles.commentAvatarText}>{item.nomeUsuario?.charAt(0).toUpperCase() || '?'}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
...
export default function FeedScreen() {
  ...
  useEffect(() => { carregarFeed(); }, []);
  const onRefresh = useCallback(() => { setRefreshing(true); carregarFeed(); }, []);

  const handleCurtir = async (postId: number) => {
    try {
      const updatedPost = await postService.curtir(postId);
      setPosts(prevPosts => prevPosts.map(post => post.id === postId ? { ...updatedPost, curtidoPorMim: !post.curtidoPorMim } : post));
    } catch (error) {
  ...
  const handleComentar = (postId: number) => {
    setPosts(prevPosts => prevPosts.map(post => post.id === postId ? { ...post, totalComentarios: post.totalComentarios + 1 } : post));
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <FlatList
        data={posts}
        keyExtractor={(item) => item.id.toString()}
        renderItem={({ item }) => <PostItem post={item} onCurtir={handleCurtir} onComentar={handleComentar} />}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity style={[styles.modalButton, styles.submitButton]} onPress={handleCriarPost}>
...
const styles = StyleSheet.create({
  ...
  postButton: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.surfaceHigh, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 30 },
  postButtonText: { color: colors.primary, fontWeight: '600', fontSize: 14 },
  listContent: { padding: 16, gap: 16, paddingBottom: 40 },
  postCard: { backgroundColor: colors.surface, borderRadius: 20, padding: 16, gap: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 6, elevation: 3 },
  ...
  modalContent: { backgroundColor: colors.surface, borderRadius: 24, padding: 20, width: '90%', gap: 16 },
  modalTitle: { fontSize: 20, fontWeight: '700', color: colors.text, textAlign: 'center' },
  modalInput: { backgroundColor: colors.surfaceHigh, borderRadius: 16, padding: 12, color: colors.text, fontSize: 16, minHeight: 100, textAlignVertical: 'top' },
  modalButtons: { flexDirection: 'row', gap: 12, justifyContent: 'flex-end' },
  modalButton: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 30 },
  cancelButton: { backgroundColor: colors.surfaceHigh },
